...
import { CertProvider, useCert } from './context/cert.CertContext';
...
const CertLayout = () => {
  const { keys, loading } = useCert();
  const location = useLocation();
  const hasRootCA = keys.some(k => k.caType === 'ROOT');
  ...
  // Block access to CSR and Issuing CA pages if Root CA is not set up
  if (!hasRootCA && (location.pathname.includes('/csr') || location.pathname.includes('/internal-ca'))) {
    return <Navigate to="/cert-manager" replace />;
  }

  return (
    <>
      <Outlet />
      {!hasRootCA && <SetupRootCAModal />}
    </>
  );
};
